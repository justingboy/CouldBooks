...
public class AESHelper {
	...
	public boolean AESCipher(int cipherMode, String sourceFilePath,
		...
		try {
			...
			Cipher mCipher = Cipher.getInstance(AESCrypt.AES_MODE);
			...
			SecretKeySpec secretKey = AESCrypt.generateKey("hyy001");
			...
			mCipher.init(cipherMode, secretKey, new IvParameterSpec(ivBytes));
			...
			while (sourceFC.read(byteData) != -1) {
				...
				// 将缓冲区准备为数据传出状态
				byteData.flip();
				// Base64.decode(byteData.toString(),Base64.NO_WRAP);
...
//				byte[] base64Array = Base64.decode(byteList, Base64.NO_WRAP);
				byte[] bytes = mCipher.doFinal(byteList);
				// Base64.decode(bytes.toString(),Base64.NO_WRAP);
