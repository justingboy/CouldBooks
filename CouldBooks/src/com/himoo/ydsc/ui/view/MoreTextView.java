...
public class MoreTextView extends LinearLayout {
	...
	protected void initalize() {
		...
		expandView.setPadding(padding, padding, padding, 0);
		...
		setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (isExpand) {
					...
					deltaValue = contentView.getLineHeight() * maxLine
							- startValue;
	...
	public void setText(CharSequence charSequence) {
		...
		if (charSequence != null && charSequence.length() > 110) {
			...
			if (charSequence != null && charSequence.length() < 40) {
				textHeight = contentView.getLineHeight() * 2;
			} else if (charSequence != null && charSequence.length() < 66) {
				textHeight = contentView.getLineHeight() * (maxLine - 2);
			} else {
				textHeight = contentView.getLineHeight() * (maxLine - 1);
			}
			contentView.setHeight(textHeight);
